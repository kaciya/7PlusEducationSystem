 ...
 */
export function useEditRS(editModalVisible, getQuestion, questionDetail, uploadAudioList) {
  // 表单数据 校验规则
  const editRS = reactive({
    model: {
      // 题目id
      id: "",
      // 编号
  ...
  watch(questionDetail, (val) => {
    if (editModalVisible.rs) {
      for (const key in val) {
        if (key == "labels") {
          // 标签特殊处理，将labels:[{id:1, name:'高频'}] map为 表单中的labelIds:['1']
          editRS.model.labelIds = val[key].map((value) => value.id);
        }
        else {
          // 其它值直接赋值
          editRS.model[key] = val[key]
        }
      }
    }
  })
  ...
  const confirmEditRS = () => {
    // 先校验
    editRSRef.value.validate().then(() => {
      // 发送编辑题目请求
      httpPost(speak.EditQuestion('rs'), editRS.model).then((res) => {
        if (res.success == true) {
          // 提示用户编辑成功
          ...
          getQuestion();
          // 关闭RS模态框
          editModalVisible.rs = false;
          // 重置表单
          editRSRef.value.resetFields();
          // 清除音频上传列表
