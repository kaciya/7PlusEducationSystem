...
export function useSetFragment(fData) {
    ...
    function dragFragmentDotEvent(audio, dot) {
        ...
        function fMove(e) {
            if (flag) {
                ...
                // 获取进度条的总宽度[number]
                const pgsWidth = parseFloat(window.getComputedStyle(progressBar, null).width.replace("px", ""));
    ...
    function updateFData(audio, dot) {
        ...
        if ((bLeft <= eLeft && dot.dataset["left"] === "bLeft") || (bLeft >= eLeft && dot.dataset["left"] === "eLeft")) {
            // 赋值起始时间（左）
