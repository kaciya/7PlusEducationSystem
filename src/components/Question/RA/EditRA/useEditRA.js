 ...
 */
export function useEditRA(editModalVisible, getQuestion, questionDetail, uploadAudioList) {
  // 表单数据 校验规则
  ...
  const confirmEditRA = () => {
    // 先校验
    editRARef.value.validate().then(() => {
      // 发送编辑题目请求
      httpPost(speak.EditQuestion('ra'), editRA.model).then((res) => {
        if (res.success == true) {
          // 提示用户编辑成功
