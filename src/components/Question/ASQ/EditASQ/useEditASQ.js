 ...
 */
export function useEditASQ(editModalVisible, getQuestion, questionDetail, uploadAudioList) {
  // 表单数据 校验规则
  ...
  const confirmEditASQ = () => {
    // 先校验
    editASQRef.value.validate().then(() => {
      // 发送编辑题目请求
      httpPost(speak.EditQuestion('asq'), editASQ.model).then((res) => {
        if (res.success == true) {
          // 提示用户编辑成功
