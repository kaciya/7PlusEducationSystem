 ...
 */
export function useEditHIW(editHIW, editModalVisible, getQuestion, uploadAudioList, audioSynthetic, questionDetail) {
    ...
    if (editModalVisible.hiw) {
      ...
      for (const key in val) {
        if (key == "labels") {
          ...
          // 设置错误展示
          val[key].forEach(ele => {
            allTitleText.value[allTitleText.value.findIndex((value) => value.answer == ele.answer)].text = ele.text;
          });
        }
        else {
          // 其它值直接赋值
  ...
  // 题目原文及答案
  const allTitleText = computed(() => editHIW.model.titleQuestion.split(' ').map(val => ({
    text: "",
    answer: val
  })))
  ...
  const confirmEditHIW = () => {
    // 先校验
    editHIWRef.value.validate().then(async () => {
      // 有原文内容且没有上传音频
      if (editHIW.model.titleQuestion.trim().length > 0 && editHIW.model.titleAudio.length == 0) {
        // 自动将原文转音频
      ...
      // 发送编辑题目请求
      httpPost(listen.EditQuestion('hiw'), editHIW.model).then((res) => {
        if (res.success == true) {
          // 提示用户编辑成功
