...
<div>
  <!-- 页头 start -->
  <a-row>
    <a-col :span="24"><a-button type="primary" style="float: right;margin-bottom: 15px" @click="showAddModal">添加广告图</a-button></a-col>
  </a-row>
  <!-- 页头 end -->
  <!-- 表格 start -->
  <a-table
  ...
  >
    ...
    <template #operational="{ record }">
      ...
      <a-popconfirm placement="topRight" title="您真的要禁用么?" @confirm="changePosterList(record.id)" v-if="record.enabled === 1">
        <a-button size="small" type="danger" style="margin: 0 10px">禁用</a-button>
      </a-popconfirm>
      <a-button size="small" v-else style="margin: 0 10px" type="primary" @click="changePosterList(record.id)">启用</a-button>
...
export default {
  ...
  setup() {
    ...
    //#region 添加学院海报列表
    const { addModalVisible,addRef,addModel,addRule,showAddModal,addSubmit,addCancel } = useAddPosterList(store,getPosterList);
    //#endregion

    //#region 编辑学院海报列表
    const { editModalVisible,editRef,editModel,showEditModal,editSubmit,editCancel } = useEditPosterList(store,getPosterList);
    //#endregion
