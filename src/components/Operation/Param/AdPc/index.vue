...
<div>
  ...
  >
    ...
    <template #operational="{ record }">
      ...
      <a-popconfirm placement="topRight" title="您真的要禁用么?" @confirm="changeAdPcList(record.id)" v-if="record.enabled === 1">
        <a-button size="small" type="danger" style="margin: 0 10px">禁用</a-button>
      </a-popconfirm>
      <a-button size="small" v-else style="margin: 0 10px" type="primary" @click="enableAdPcList(record.id)">启用</a-button>
...
export default {
  ...
  setup() {
    ...
    //#region 添加广告方法
    const { addModalVisible,addRef,addModel,addRule,showAddModal,addSubmit,addCancel } = useAddAdPcList(store,getAdPcList);
    //#endregion

    //#region 编辑广告方法
    const { editModalVisible,editRef,editModel,showEditModal,editSubmit,editCancel } = useEditAdPcList(store,getAdPcList);
    //#endregion
