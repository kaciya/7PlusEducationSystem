...
<!-- 头部 start -->
  <a-row>
    <a-col :span="24" style="margin-bottom: 15px">
      <a-button type="primary" style="float: right;" @click="showAddModal">添加</a-button>
    </a-col>
  </a-row>
<!-- 头部 end -->
<!-- 表格 start -->
  ...
  >
    ...
    <template #operational="{ record }">
      <a-button class="modify-btn" type="primary" style="margin-right: 10px" size="small" @click="showEditModal(record.id)">重新上传</a-button>
      <a-popconfirm placement="topRight" @confirm="delSubmit(record.id)" @cancel="delCancel" title="您真的要删除吗?">
...
export default {
  ...
  setup() {
    ...
    //#region 添加校园风采
    const { addModalVisible,showAddModal,addSubmit,addCancel } = useAddCampStyleList(store,getCampStyleList);
    //#endregion

    //#region 重新上传功能
    const { editModalVisible,showEditModal,editSubmit,editCancel } = useEditCampStyleList(store,getCampStyleList);
    //#endregion
